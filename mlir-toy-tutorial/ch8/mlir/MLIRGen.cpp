...
private:
  ...
  mlir::Value mlirGen(CallExprAST &call) {
    ...
    // toy.sin: element-wise sine via 6-term Taylor series lowering pass.
    if (callee == "sin") {
      if (call.getArgs().size() != 1) {
        emitError(location, "MLIR codegen encountered an error: toy.sin "
                            "does not accept multiple arguments");
        return nullptr;
      }
      return SinOp::create(builder, location, operands[0]);
    }
